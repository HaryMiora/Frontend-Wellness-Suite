  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FAFAF9] to-[#E5E7EB] flex items-center justify-center font-sans">
      <div className="max-w-sm w-full p-5 bg-white rounded-2xl shadow-2xl">
        <h2 className="text-2xl font-bold text-black mb-2 text-center">Créer un compte</h2>
        <p className="text-center text-gray-600 mb-5 text-sm">
          Rejoindre <span className="font-semibold text-[#BECDAC]">Wellness Suite</span>
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="fullName" className="block text-xs font-medium text-black mb-1">Nom complet</label>
            <input
              type="text"
              id="fullName"
              value={fullName}
          ...
          <div>
            <label htmlFor="confirmPassword" className="block text-xs font-medium text-black mb-1">Confirmer le mot de passe</label>
            <input
              type="password"
              id="confirmPassword"
        ...
        <p className="mt-3 text-center text-xs text-gray-600">
          Déjà un compte ?{' '}
          <a href="/login" className="text-[#BECDAC] hover:underline">Se connecter</a>
        </p>
      </div>
    </div>
  );
